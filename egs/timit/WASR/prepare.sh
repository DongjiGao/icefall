...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  lhotse prepare timit -p $num_phones -j $nj "$dl_dir/timit/data" "${manifests_dir}"
fi

if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  log "Stage 2: Compute SSL feature for timit"
  mkdir -p "${feature_dir}"
  ./local/compute_ssl_timit.py
fi

lang_dir="data/lang_phone_${wasr_type}"
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  log "Stage 3: Prepare phone based WS lang (${wasr_type})"
  mkdir -p "${lang_dir}"

  ./local/prepare_ws_lexicon.py \
   --manifests-dir "${manifests_dir}" \
   --lang-dir "${lang_dir}"

  if [ ! -f "${lang_dir}/L_disambig.pt" ]; then
    ./local/prepare_ws_lang.py --lang-dir "${lang_dir}"
  fi
fi
